...
Write $p(X) = Pr(Y=1|X)$, logistic regression uses the form $p(X) = \frac{e^{\beta_0 + \beta_1 X_1 + ... + \beta_p X_p}}{1 + e^{\beta_0+\beta_1 X + ... +\beta_p X_p}}$. By applying the logit transformation, $log(\frac{p(X)}{1-p(X)}) = \beta_0 + \beta_1 X_1 + ... + \beta_p X_p$, we use maximum likelihood to estimate the parameters.\par
