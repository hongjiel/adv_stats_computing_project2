...
As exploring the breast cancer data, there are many predictors are highly correlated to each other as shown in the Figure \ref{Figure 1.1}, such as 'area_mean', 'perimeter_worst', 'radius_mean' and so on, that could cause unstable parameter estimation as well as perplexing the interpretation of logistic model. While we could eliminate some correlated features for modeling as shown in the Figure \ref{Figure 1.2}, regularized logistic regression also help to tackle with it, which would be further explore in the following parts. 
...
```{r echo=FALSE, fig.cap = "\\label{Figure 1.1} Variables Correlation Part I"}
corr
```

```{r echo=FALSE, fig.cap = "\\label{Figure 1.2} Variables Correlation Part II"}
corr1
```
