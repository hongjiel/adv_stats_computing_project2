...
We select the best $\lambda$ using our own function (see Figure 4) and `cv.glmnet`. Both our cross-validation function and `cv.glmnet` function reach the greatest mean AUC when $\lambda=0.0099322$ (see Figure 5), thus this is the best $\lambda$ we choose. As for the predictors, when $\lambda = \lambda_{best}$, both methods select ten same predictors, which are `texture_mean`, `concave.points_mean`, `radius_se`, `fractal_dimension_se`, `radius_worst`, `texture_worst`, `smoothness_worst`, `concavity_worst`, `concave.points_worst`, and `symmetry_worst` (see Figure 6).
