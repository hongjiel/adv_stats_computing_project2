...
First, we prove that: there exists a vector of coefficients $\tilde{\boldsymbol{\beta}}$ such that $Y_i = 1$ whenever $\mathrm{x}_i^\top\tilde{\boldsymbol{\beta}} > 0$ and $Y_i = 0$ whenever $\mathrm{x}_i^\top\tilde{\boldsymbol{\beta}} < 0$.

Let $A_1=\{i: Y_i = 1\}= \{i:\mathrm{x}_i^\top\hat{\boldsymbol{\beta}}> 0\}$ and $A_0=\{i: Y_i = 0\}=\{i:\mathrm{x}_i^\top\hat{\boldsymbol{\beta}}\le0\}$. Then we have $$\epsilon:=\min_{i\in A_1}(\mathrm{x}_i^\top\hat{\boldsymbol{\beta}}) > 0.$$
Let $\tilde{\boldsymbol{\beta}}=\hat{\boldsymbol{\beta}}-(\epsilon/2,0,\ldots,0)^\top$. Given that $X_{i1}=1$ for all $i$, we have 
$$\begin{aligned}
\mathrm{x}_i^\top\tilde{\boldsymbol{\beta}}=\mathrm{x}_i^\top\hat{\boldsymbol{\beta}}-\epsilon/2\cdot1\ge \epsilon-\epsilon/2=\epsilon/2>0,\quad \forall i\in A_1\\
\mathrm{x}_i^\top\tilde{\boldsymbol{\beta}}=\mathrm{x}_i^\top\hat{\boldsymbol{\beta}}-\epsilon/2\cdot1\le 0-\epsilon/2=-\epsilon/2<0,\quad \forall i\in A_0\\
\end{aligned}$$
Thus we have $A_1=\{i: Y_i = 1\}= \{i:\mathrm{x}_i^\top\tilde{\boldsymbol{\beta}}> 0\}$ and $A_0=\{i: Y_i = 0\}=\{i:\mathrm{x}_i^\top\tilde{\boldsymbol{\beta}}< 0\}$.

Next, we prove that 
\begin{equation}\label{loglik}
\lim_{k\rightarrow\infty}f(k\tilde{\boldsymbol{\beta}})=0.
\end{equation}
$\forall k>0$, we have $A_1= \{i:\mathrm{x}_i^\top(k\tilde{\boldsymbol{\beta}})> 0\}$ and $A_0==\{i:\mathrm{x}_i^\top(k\tilde{\boldsymbol{\beta}})< 0\}$.
