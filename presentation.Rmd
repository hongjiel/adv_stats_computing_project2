...
The corresponding R-function for the algorithm
```{r eval=FALSE, size = "tiny"}
...
## Task 2 - Comparison
When there is no complete separation, the parameters output by `glm` function and our algorithm are the same.


## Task 2 - Comparison
Comparison of using `glm` function and our algorithm (part1)
...
c_t[1:15, ] %>% knitr::kable()
```


## Task 2 - Comparison
Comparison of using `glm` function and our algorithm (part2)
```{r echo = FALSE, warning = FALSE, size = "tiny"}
c_t[16:31, ] %>% knitr::kable()
```
