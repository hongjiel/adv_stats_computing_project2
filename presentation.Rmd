...
- A complete separation in a logistic regression, sometimes also referred as perfect prediction, occurs whenever there exists some vector of coefficients $\boldsymbol{\beta}$ such that $Y_i = 1$ whenever $\mathbf{x}_i^\top\boldsymbol{\beta} > 0$ and $Y_i = 0$ whenever $\mathbf{x}_i^\top\boldsymbol{\beta} \le 0$. 
...
Based on current parameter estimates $\tilde{\boldsymbol{\beta}}$, we we form a quadratic approximation to the log-likelihood $f$ using a Taylor expansion:
...
## Task 3 - INNER LOOP
**INNER LOOP: **\par
With fixed $\tilde{w}_i$'s, $\tilde{z}_i$'s, and a fixed form of $\ell$ based on the estimates of $\boldsymbol{\beta}$ in the previous iteration of the middle loop, we use coordinate descent to update $\boldsymbol{\beta}$ by solving
...
Based the current estimates $\tilde{\beta}_k$ for $k\ne j$:
...
## Task 3 - Path-wise coordinate-wise optimization algorithm
\begin{figure}[H] 
...
1. Hothorn et al. The design and analysis of benchmark experiments. Journal of Computational and Graphical Statistics (2005) vol. 14 (3) pp. 675-699
